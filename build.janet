...
## Runda 35 -- DBus (nagłówki + pkg-config), potrzebne przez
## shell/dbus_notify_shim.c + shell/dbusnotify.nim (integracja
## powiadomień z org.freedesktop.Notifications, patrz "Nowości w v0.2
## (runda 35)" w README.md). W ODRÓŻNIENIU od pozostałych zależności
## `check-shell-common-deps` niżej, to jest zależność potrzebna WYŁĄCZNIE
## do KOMPILACJI (nagłówki `dbus/dbus.h` + `pkg-config --cflags/--libs
## dbus-1`) -- w RUNTIME, gdy sesja nie ma działającego `dbus-daemon
## --session`, ta integracja po prostu się nie aktywuje (patrz duży
## komentarz w dbusnotify.nim), więc brak tego pakietu jest sprawdzany
## tutaj jako TWARDY wymóg budowania, nie jako miękka, opcjonalna
## zależność runtime jak `dwebp`/`wl-copy`/`xclip`.
(def dep-dbus-dev
  {:apt ["libdbus-1-dev"] :dnf ["dbus-devel"] :pacman ["dbus"]
   :zypper ["dbus-1-devel"] :apk ["dbus-dev"]})
...
(defn check-shell-common-deps []
  ...
  ## Runda 35 -- patrz duży komentarz przy dep-dbus-dev wyżej. Sprawdzane
  ## tu (nie w check-system-deps) bo, tak jak biblioteki X11/GLFW/GL
  ## powyżej, dotyczy KAŻDEGO zde-shell (dowolny backend), nie tylko
  ## zde-comp/Wayland -- shell/notifications.nim jest importowane zawsze.
  (ensure-pkgconfig ["dbus-1"] dep-dbus-dev "dbus-1"
    "zainstaluj nagłówki deweloperskie DBus (libdbus-1-dev / dbus-devel / dbus) ręcznie.")
  (log "OK: DBus (integracja powiadomień, runda 35)."))
